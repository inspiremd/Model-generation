...
if [ $# -ne 1 ];then
  ...
  for ((n=1; n<=num_reps; n++)); do
    ...
    awk '($1=="BOND"||$1=="VDWAALS"||$1=="1-4")' $dir_in/_MMPBSA_complex_pb.mdout.all | cut -c11-24,36-49,64-77 | awk -f $SCRIPTS_PATH/t-ser-mmpb.awk > $dir_out/mmpb_lig.dat
    awk '($1=="BOND"||$1=="VDWAALS"||$1=="1-4")' $dir_in/_MMPBSA_complex_gb.mdout.all | cut -c11-24,36-49,64-77 | awk -f $SCRIPTS_PATH/t-ser-mmpb.awk > $dir_out/mmgb_lig.dat
    awk '($1 != "#Frame"){print $2*0.0072}' $dir_in/_MMPBSA_complex_gb_surf.dat.all > $dir_out/esurf_lig.dat
